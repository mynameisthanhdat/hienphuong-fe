...
import { getAdminSession, isAdminAuthenticated, logoutAdmin } from '../../services/adminAuth';
...
const ProtectedAdminLayout: React.FC = () => {
  ...
  if (!isAdminAuthenticated()) {
    return <Navigate to="/private/hp/admin" replace state={{ from: location.pathname }} />;
  }
        ...
        <aside className="border-b border-[#d7c8b5] bg-[#2f241c] px-6 py-6 text-white lg:min-h-screen lg:w-[300px] lg:border-b-0 lg:border-r lg:px-7 lg:py-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-serif font-bold">{t('admin.layout.title')}</h1>
            </div>
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <p className="mt-2 text-lg font-semibold text-white">{session?.name ?? session?.username ?? 'admin'}</p>
            {session?.name && <p className="mt-1 text-sm text-white/60">@{session.username}</p>}
            <p className="mt-1 text-sm text-white/60">{t('admin.layout.sessionActive')}</p>
          </div>
