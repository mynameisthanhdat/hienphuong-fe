...
import { Menu, X, Phone, Home, Info, Map, Bed, CalendarCheck, Search } from 'lucide-react';
...
const Navigation: React.FC = () => {
  ...
  const mobileNavLinks = [...navLinks, { name: t('navigation.reservation'), path: '/reservation', icon: CalendarCheck }];
  ...
  return (
    <nav className={headerClass}>
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-20' : 'h-28'}`}>
          ...
          <Link to="/" className="flex-shrink-0 flex items-center gap-2 group">
             <img 
               src="https://res.cloudinary.com/dovk5x2wb/image/upload/v1741012345/hien_phuong_logo_yellow.png" 
            ...
            <div className={`flex flex-col justify-center ${textColorClass}`}>
                <span className="text-gold-500 text-2xl md:text-3xl font-serif font-bold tracking-widest leading-none drop-shadow-md group-hover:text-gold-400 transition-colors">Hiền Phương</span>
                <span className="text-[10px] md:text-xs tracking-[0.3em] uppercase opacity-90 font-bold group-hover:text-white/80 transition-colors ml-0.5">Motel</span>
            </div>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-4">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`px-3 py-2 text-sm font-semibold tracking-wider uppercase transition-colors duration-300 ${
                    isActive(link.path) ? activeTextColorClass : `${textColorClass} ${hoverColorClass}`
                  }`}
                >
                  {link.name}
                </Link>
            ...
            <button
              ...
              className={`p-3 rounded-full ${textColorClass} hover:bg-white/10 transition-colors`}
            >
              <Search size={18} />
            </button>
      ...
      {isOpen && (
        <div className={`md:hidden pb-4 border-t border-white/10 ${scrolled ? 'bg-brown-900/90' : 'bg-black/90 backdrop-blur-md'}`}>
