...
const getRoomFallbackImage = (roomId: string) =>
  `https://picsum.photos/600/400?random=${roomId}`;
...
const Rooms: React.FC = () => {
  ...
  const [roomsPayload, setRoomsPayload] =
    useState<RoomAvailabilityPayload | null>(null);
  const [isLoading, setIsLoading] = useState(hasRoomsApiUrl);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [reloadKey, setReloadKey] = useState(0);
  const [hasResolvedInitialFilters, setHasResolvedInitialFilters] =
    useState(false);
  ...
  const [typeFilter, setTypeFilter] = useState<"ALL" | "Single" | "Double">(
    "ALL",
  );
  const [rankFilter, setRankFilter] = useState<"ALL" | "Standard" | "VIP">(
    "ALL",
  );
  ...
  const FilterGroup = ({
    ...
    <div className="flex flex-col items-center md:items-start gap-2">
      <span className="text-xs font-bold text-brown-400 uppercase tracking-widest">
        {title}
      </span>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col md:flex-row justify-center items-center gap-6 mb-16">
              ...
              <div className="flex gap-4">
                <a
                  href="tel:0345011468"
                  className="inline-flex items-center gap-2 bg-white border border-gold-500 text-gold-600 px-5 py-2 rounded-full font-bold hover:bg-gold-500 hover:text-white transition shadow-sm"
                >
                  <Phone size={16} /> 0345 011 468
                </a>
                <a
                  href="tel:0377586258"
                  className="inline-flex items-center gap-2 bg-brown-800 text-white px-5 py-2 rounded-full font-bold hover:bg-brown-900 transition shadow-sm"
                >
            ...
            {isLoading && hasRoomsApiUrl && !roomsPayload && (
              <div className="mb-10 rounded-2xl border border-brown-100 bg-white px-6 py-12 text-center shadow-lg">
                <p className="text-2xl font-serif font-bold text-brown-900">
                  {t("rooms.loadingTitle")}
                </p>
                <p className="mt-3 text-brown-500">
                  {t("rooms.loadingDescription")}
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
              {filteredRooms.map((room) => (
                <div
                  key={room.id}
                  className="bg-white rounded-xl overflow-hidden shadow-lg border border-brown-100 hover:shadow-2xl transition duration-500 group flex flex-col h-full transform hover:-translate-y-2"
                >
                  {/* Image Area */}
                  <div className="h-64 overflow-hidden relative">
                    <button
                      ...
                      aria-label={t("rooms.previewAria", {
                        roomName: room.name,
                      })}
                    >
                      <img
                        src={
                          room.thumbnail ||
                          room.images?.[0] ||
                          getRoomFallbackImage(room.id)
                        }
                        alt={t("rooms.photoAlt", { roomName: room.name })}
                        className="w-full h-full object-cover group-hover:scale-110 transition duration-700 ease-in-out"
                      />
                      <div className="absolute inset-0 flex items-center justify-center bg-[#1f1712]/45 opacity-100 transition duration-300 md:bg-[#1f1712]/10 md:opacity-0 md:group-hover:bg-[#1f1712]/45 md:group-hover:opacity-100">
                        <span className="rounded-full border border-white/40 bg-white/10 px-5 py-2 text-sm font-bold uppercase tracking-[0.24em] text-white backdrop-blur-sm shadow-lg">
                          {t("rooms.preview")}
                        </span>
                      </div>
                    </button>
                  ...
                  <div className="p-8 flex flex-col flex-grow">
                    ...
                    <div className="mb-3 flex flex-wrap gap-2">
                      <span className="text-xs font-bold text-gold-600 tracking-[0.2em] uppercase border border-gold-200 px-3 py-1 rounded-full bg-gold-50">
                        ...
                        {getRoomCategoryBadge(room.category)}
                      </span>
                      <span
                        className={`text-xs font-bold tracking-[0.2em] uppercase border px-3 py-1 rounded-full ${getRoomStatusMeta(room.booking_status).className}`}
                      >
                    ...
                    <div className="mb-6 flex items-baseline gap-1">
                      ...
                      <span className="text-sm text-brown-500 font-light">
                        {t("rooms.perNight")}
                      </span>
                    </div>
                    ...
                    <Link
                      to="/reservation"
                      className={`${room.booking_status === "booked" ? "opacity-50 pointer-events-none cursor-not-allowed" : ""} w-full py-4 bg-brown-900 text-white font-bold rounded-lg hover:bg-gold-600 transition shadow-lg text-center uppercase tracking-widest text-sm flex items-center justify-center gap-2 group-hover:scale-105 transform duration-300`}
                    >
                      {t("rooms.bookNow")}
                    </Link>
                  </div>
            ...
            {!isLoading && !loadError && filteredRooms.length === 0 && (
              <div className="text-center py-20">
                <p className="text-brown-500 text-xl font-serif">
                  {t("rooms.noResults")}
                </p>
                <button
                  type="button"
